...
#define ECS_RUN_ITER ECS_SYSTEM; template <typename Builder> static __forceinline void run
#define ECS_ITERABLE(q) QueryIterable<q, Builder>
...
struct UpdatePositionExternalQuery
{
  ...
  ECS_RUN_ITER(const UpdateStage &stage, ECS_ITERABLE(AliveEnemy) enemies)
  {
    const int count = enemies.count();

    for (AliveEnemy enemy : enemies)
    {
    }
  }
};
...
int main(int argc, char *argv[])
{
  ...
  //   UpdatePositionExternalQuery::run(*(UpdateStage*)nullptr, QueryIterable<AliveEnemy, AliveEnemyBuilder>(query));

  //   for (auto q = query.begin(), e = query.end(); q != e; ++q)
  //   {
  //     UpdatePositionExternalQuery::run(*(UpdateStage*)nullptr,
  //     {
  //       q.get<EntityId>(INDEX_OF_COMPONENT(AliveEnemy, eid)),
  //       q.get<glm::vec4>(index_of_component<_countof(AliveEnemy_components)>::get(HASH("collision_rect"), AliveEnemy_components)),
  //       q.get<glm::vec2>(index_of_component<_countof(AliveEnemy_components)>::get(HASH("pos"), AliveEnemy_components)),
  //       q.get<glm::vec2>(index_of_component<_countof(AliveEnemy_components)>::get(HASH("vel"), AliveEnemy_components)),
  //       q.get<bool>(index_of_component<_countof(AliveEnemy_components)>::get(HASH("is_alive"), AliveEnemy_components))
  //     });
  //   }
  // }
